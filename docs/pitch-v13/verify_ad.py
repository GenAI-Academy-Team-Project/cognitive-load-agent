...
for name,size in [('carestead-ad-v13.mp4','1920x1080'),('carestead-ad-v13.webm','1280x720')]:
...
r=subprocess.run([ff,'-hide_banner','-i',str(R/'carestead-ad-v13.mp4'),'-af','loudnorm=I=-16:TP=-1.5:LRA=9:print_format=json','-f','null','-'],capture_output=True,text=True);stats=json.loads(r.stderr[r.stderr.rfind('{'):r.stderr.rfind('}')+1]);assert -18<float(stats['input_i'])<-14;assert float(stats['input_tp'])<=-1
