...
export async function GET(request: Request) {
  ...
  try {
    ...
    const payload = { schema_version: 'carestead-export-v2', exported_at: completedAt, exported_by: auth.member.email, non_clinical_notice: 'Carestead is a care-coordination tool, not a medical record or clinical decision system.', recipient, profile, consent, support_contacts: contacts, care_team: careTeam, plans, tasks, events, memories, risks, approvals, traces, notifications, chat_threads: chatThreads, chat_messages: chatMessages, chat_actions: chatActions };
