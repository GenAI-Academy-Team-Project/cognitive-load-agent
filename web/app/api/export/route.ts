...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [profile, contacts, plans, tasks, events, memories, risks, approvals, traces, notifications, careTeam, chatThreads, chatMessages, chatActions] = await Promise.all([
      env.DB.prepare('SELECT * FROM recipient_profiles WHERE recipient_id=?').bind(recipientId).first(), all('SELECT * FROM support_contacts WHERE recipient_id=?', [recipientId]), all('SELECT * FROM care_plans WHERE recipient_id=?', [recipientId]), all(scoped('tasks','task'), [recipientId]), all(scoped('events','event'), [recipientId]), all(scoped('memories','memory'), [recipientId]), all(scoped('risks','risk'), [recipientId]), all(scoped('approvals','approval'), [recipientId]), all(scoped('traces','trace'), [recipientId]), all('SELECT * FROM notifications WHERE recipient_id=?', [recipientId]), all('SELECT c.display_name,c.email,rm.access_role role,c.status FROM recipient_members rm JOIN care_circle_members c ON c.id=rm.member_id WHERE rm.recipient_id=?', [recipientId]), all('SELECT id,title,created_at,updated_at FROM chat_threads WHERE recipient_id=?', [recipientId]), all('SELECT id,thread_id,role,content,evidence_json,action_request_id,created_at FROM chat_messages WHERE thread_id IN (SELECT id FROM chat_threads WHERE recipient_id=?)', [recipientId]), all('SELECT id,thread_id,action_type,summary,payload_json,status,requires_approval,created_at,decided_at,executed_at FROM chat_action_requests WHERE recipient_id=?', [recipientId]),
    ]);
    const calendarAppointments = await all('SELECT * FROM calendar_appointments WHERE recipient_id=?', [recipientId]);
    const calendarActions = await all('SELECT id,kind,payload_json,status,error,html_link,created_at,updated_at FROM calendar_actions WHERE recipient_id=?', [recipientId]);
    const memoryIntegration = await env.DB.prepare('SELECT enabled,remote_dirty,deletion_event,updated_at FROM memory_integrations WHERE recipient_id=?').bind(recipientId).first();
    const planning = Object.fromEntries(await Promise.all(planningTables.map(async (table) => [table, await all(`SELECT * FROM ${table} WHERE recipient_id=?`, [recipientId])])));
    const notificationDeliveries = await all('SELECT * FROM notification_deliveries WHERE recipient_id=?', [recipientId]);
    const notificationReads = await all('SELECT r.* FROM notification_reads r JOIN notifications n ON n.id=r.notification_id WHERE n.recipient_id=?', [recipientId]);
    const notificationPreferences = await all('SELECT recipient_id,member_id,email_enabled,sms_enabled,phone,updated_at FROM notification_preferences WHERE recipient_id=?', [recipientId]);
    ...
    const pushoverPreferences = await all('SELECT recipient_id,member_id,updated_at FROM pushover_preferences WHERE recipient_id=?', [recipientId]);
    const payload = { pushover_preferences: pushoverPreferences, schema_version: 'carestead-export-v6', notification_deliveries: notificationDeliveries, notification_reads: notificationReads, notification_preferences: notificationPreferences, planning, exported_at: completedAt, exported_by: auth.member.email, non_clinical_notice: 'Carestead is a care-coordination tool, not a medical record or clinical decision system.', recipient, profile, consent, memory_integration: memoryIntegration, support_contacts: contacts, care_team: careTeam, plans, tasks, events, memories, risks, approvals, traces, notifications, chat_threads: chatThreads, chat_messages: chatMessages, chat_actions: chatActions, calendar_appointments: calendarAppointments, calendar_actions: calendarActions };
