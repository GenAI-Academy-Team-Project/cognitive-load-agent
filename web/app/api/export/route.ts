...
export async function GET(request: Request) {
  ...
  try {
    ...
    const ntfyPreferences = await all('SELECT recipient_id,member_id,updated_at FROM ntfy_preferences WHERE recipient_id=?', [recipientId]);
    const payload = { ntfy_preferences: ntfyPreferences, schema_version: 'carestead-export-v6', notification_deliveries: notificationDeliveries, notification_reads: notificationReads, notification_preferences: notificationPreferences, planning, exported_at: completedAt, exported_by: auth.member.email, non_clinical_notice: 'Carestead is a care-coordination tool, not a medical record or clinical decision system.', recipient, profile, consent, support_contacts: contacts, care_team: careTeam, plans, tasks, events, memories, risks, approvals, traces, notifications, chat_threads: chatThreads, chat_messages: chatMessages, chat_actions: chatActions, calendar_appointments: calendarAppointments, calendar_actions: calendarActions };
