...
export function CareChat({ recipientId, recipientName, canWrite, onActionCompleted }: { recipientId: string; recipientName: string; canWrite: boolean; onActionCompleted: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/chat?recipientId=${encodeURIComponent(recipientId)}`)
      ...
      .then((result) => { if (active) setChat(result); })
        ...
        <SheetContent className="w-[min(100vw,460px)] gap-0 border-l-0 bg-[#f7f9f7] p-0 sm:max-w-[460px]" aria-label={`Carestead assistant for ${recipientName}`}>
          <SheetHeader className="border-b bg-card px-5 py-4 pr-14">
            <div className="flex items-center gap-3">
              ...
              <div><SheetTitle className="font-heading text-lg font-semibold">Ask about {recipientName}</SheetTitle><SheetDescription className="mt-0.5 text-xs">Grounded in this profile · actions require approval</SheetDescription></div>
            </div>
          </SheetHeader>
          ...
          <div className="border-t bg-card p-4">
            ...
            {chat?.messages.length === 0 && <div className="mb-3 flex gap-2 overflow-x-auto pb-1">{chat.quickPrompts.map((prompt) => <button key={prompt} onClick={() => send(prompt)} disabled={busy} className="shrink-0 rounded-full border bg-[#f8fbf9] px-3 py-1.5 text-left text-xs text-[var(--care-success-ink)] hover:bg-secondary disabled:opacity-50">{prompt}</button>)}</div>}
