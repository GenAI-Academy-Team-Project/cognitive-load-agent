...
export function PrivateInput({ value, kind = 'secret', label, ...props }: Omit<ComponentProps<typeof Input>, 'value'> & { value: string; kind?: MaskKind; label: string }) {
  ...
  return <span className="flex min-w-0 items-center gap-1">
    <Input {...props} onChange={(event) => { if (kind !== 'secret') setVisible(true); props.onChange?.(event); }} value={masked ? mask(value, kind) : value} type={visible ? (kind === 'phone' ? 'tel' : 'text') : kind === 'secret' ? 'password' : 'text'} readOnly={masked || props.readOnly} autoComplete="off" className="min-w-0 flex-1" />
    <RevealButton label={label} visible={visible} disabled={props.disabled} onClick={() => setVisible(!visible)} />
  </span>;
}
