...
export async function notificationTarget(db: D1Database, config: NotificationConfig, recipientId: string, input: NotificationInput) {
  ...
  if (input.channel === 'email') {
    ...
    if (!saved || !validNtfyTopic(saved.topic) || saved.server_url !== ntfyServerUrl(config.NTFY_SERVER_URL)) throw new AppError('channel_disabled', 409, 'This caregiver must enable ntfy in Notifications first.');
...
export async function prepareNotification(db: D1Database, config: NotificationConfig, recipientId: string, value: unknown) {
  ...
  return {
    ...
    payload: { ...input, targetName: target.member.display_name, destination: await destinationFingerprint(target.destination), destinationLabel: input.channel === 'ntfy' ? 'Registered ntfy topic' : input.channel === 'push' ? 'Registered browser' : target.destination },
  };
}
...
export function parseNotificationRequest(message: string) {
  ...
  const match = message.match(/^(?:please\s+)?(?:send\s+)?(email|sms|text|push|ntfy|in-app)(?:\s+notification(?=\s+to\s))?\s+(?:to\s+)?([^:]+):\s*(.+)$/i);
...
export async function executeNotification(db: D1Database, config: NotificationConfig, recipientId: string, actorMemberId: string, actionId: string, payload: Record<string, string>) {
  ...
  const claim = await db.prepare(`INSERT OR IGNORE INTO notification_deliveries (action_id,notification_id,recipient_id,member_id,channel,destination,status,created_at,updated_at)
    SELECT ?,?,?,?,?,?,'sending',?,? WHERE EXISTS (SELECT 1 FROM chat_action_requests WHERE id=? AND recipient_id=? AND actor_member_id=? AND action_type='send_notification' AND status='pending')`).bind(actionId, notificationId, recipientId, input.memberId, input.channel, input.channel === 'ntfy' ? 'Registered ntfy topic' : input.channel === 'push' ? 'Registered browser' : target.destination, now, now, actionId, recipientId, actorMemberId).run();
