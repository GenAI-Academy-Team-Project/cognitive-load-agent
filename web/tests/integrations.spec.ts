...
test('optional integrations are visible, accessible, and can be toggled when configured', async ({ page }) => {
  ...
  const filters = page.getByRole('group', { name: 'Filter Integrations', exact: true });
  await expect(filters.getByRole('combobox', { name: 'Channel / service', exact: true })).toHaveCount(0);
  await filters.getByRole('combobox', { name: 'Credential setup', exact: true }).selectOption('configured');
  await expect(sms).toBeVisible();
  await expect(calendar).toBeHidden();
  await filters.getByRole('button', { name: /Clear filters/i }).click();
  await expect(calendar).toBeVisible();
  await expect(filters.getByRole('combobox', { name: 'Connection status', exact: true })).toBeVisible();
  const manageSms = page.getByRole('button', { name: 'Manage settings for Twilio SMS', exact: true });
  await expect(manageSms).toHaveAttribute('aria-expanded', 'false');
  await manageSms.click();
  await expect(page.getByRole('region', { name: 'Your sms delivery preferences', exact: true })).toBeVisible();
  await page.getByRole('button', { name: 'Manage settings for Email delivery', exact: true }).click();
  await expect(page.getByRole('region', { name: 'Your sms delivery preferences', exact: true })).toBeHidden();
  await expect(page.getByRole('region', { name: 'Your email delivery preferences', exact: true })).toBeVisible();
  await page.getByRole('button', { name: 'Hide settings for Email delivery', exact: true }).click();
...
test('masked inputs save and reset overrides without discarding other drafts', async ({ page }) => {
  ...
  await token.fill('synthetic-private-override');
  ...
  await expect(token).toHaveValue('');
  await page.getByRole('button', { name: 'Manage settings for Twilio SMS', exact: true }).click();
  await expect(sms).toHaveValue('unsaved-secret');
  await page.getByRole('button', { name: 'Manage settings for Mobile push', exact: true }).click();
