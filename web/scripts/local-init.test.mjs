...
test('local setup creates the template once and preserves existing settings', () => {
  ...
  try {
    ...
    assert.equal(readFileSync(join(dir, '.dev.vars'), 'utf8'), 'RESEND_API_KEY=\n');
    writeFileSync(join(dir, '.dev.vars'), 'RESEND_API_KEY=keep-existing');
    assert.equal(run().status, 0);
    assert.equal(readFileSync(join(dir, '.dev.vars'), 'utf8'), 'RESEND_API_KEY=keep-existing');
